...
public struct Transaction: Encodable {
    ...
    public mutating func sign(signers: [SolanaAccount]) throws {
        ...
        // unique signers
        let signers = signers.reduce([SolanaAccount]()) { signers, signer in
    ...
    private mutating func partialSign(message: Message, signers: [SolanaAccount]) throws {
    ...
    public func compileMessage() throws -> Message {
        ...
        var accountMetas = [SolanaAccount.Meta]()
        ...
        accountMetas = accountMetas.reduce([SolanaAccount.Meta]()) { result, accountMeta in
        ...
        var uniqueMetas: [SolanaAccount.Meta] = []
...
//                        throw Error.invalidRequest(reason: "Transaction references a signature that is unnecessary")
        ...
        var signedKeys = [SolanaAccount.Meta]()
        var unsignedKeys = [SolanaAccount.Meta]()
    ...
    static func populate(_ message: Transaction.Message, _ signatures: [String]) -> Transaction {
        ...
        message.instructions.forEach { instruction in
            let keys: [SolanaAccount.Meta] = instruction.accounts.map { account in
                let pubkey = message.accountKeys[account]
                return SolanaAccount.Meta(
                    publicKey: pubkey,
