...
final class BlockchainClient {
    ...
    public init(apiClient: NetworkManager) {
    ...
    public func prepareTransaction(
        ...
        feePayer: PublicKey,
        completionHandler: @escaping (Result<PreparedTransaction, Error>) -> Void
...
//        let expectedFee = try feeCalculator.calculateNetworkFee(transaction: transaction)
        // Need rewrite getRecentBlockHash
        apiClient.recentBlockHash(completionHandler: { result in
            switch result {
            case .success(let fee):
                guard let blockhash = fee.blockhash else {
                ...
                let preparedTransaction = PreparedTransaction(transaction: transaction, signers: signers)
                completionHandler(.success(preparedTransaction))
